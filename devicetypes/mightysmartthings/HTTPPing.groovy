...
metadata {
	definition (name: "Ping (Monitor / noWOL)", namespace: "Matthias Ertl", author: "Matthias Ertl") {
		capability "Polling"
	...
	tiles {
		standardTile("contact", "device.contact", width: 3, height: 3, decoration: "flat", canChangeIcon: true, canChangeBackground: false) {
//
// color scheme is inverse of a normal sensor. 
//
            state "open", label: "ON", backgroundColor: "#79b821", icon:"st.contact.contact.open"
            state "closed", label: "OFF", backgroundColor: "#ff4444", icon:"st.contact.contact.closed"
        }
        standardTile("refresh", "device.ttl", inactiveLabel: false, decoration: "flat", width: 3, height: 1) {
            state "default", action:"polling.poll", icon:"st.secondary.refresh"
        }
        standardTile("ttl", "device.ttl", width: 3, height: 1, inactiveLabel: false, decoration: "flat") {
            state "ttl", label:"${open}"
        }
        
        standardTile("push1", "device.button", width: 1, height: 1, decoration: "flat") {
			state "default", label: "WAKE", backgroundColor: "#79b821", action: "push1"
